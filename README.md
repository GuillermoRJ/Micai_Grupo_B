...
> **Note:** Raw data files are not included in this repository due to their size (PEMS-BAY: 82 MB, US Accidents: 2.9 GB).
>
> Download them from the links above and place them in a local `data/` folder.
...
Loaded `PEMS-BAY.csv` with 52,116 timestamps and 325 sensors.
Verified absence of NaN values (0 found).
Applied forward fill and backward fill as standard cleaning protocol.
...
Implemented a detector for sudden speed drops: if the difference between two consecutive timestamps exceeds 40 km/h, the possible accident label is assigned.

**2,156 events** detected (4.96% of total).
...
The continuous traffic speed values were transformed into discrete numerical tokens using uniform quantization. This process converts numerical magnitudes into categorical representations compatible with transformer-based language models.

Each speed interval was mapped to a unique token ID, preserving temporal order while reducing numerical noise and dimensional complexity.

### 2. Semantic embeddings

Traffic incident descriptions were processed using a pretrained language model to generate semantic embeddings.

The objective of this step is to transform unstructured textual descriptions into dense vector representations capable of capturing semantic context such as congestion severity, accidents, weather influence and abnormal traffic conditions.

### 3. Multimodal fusion

A unified multimodal package was constructed for every timestamp by combining:

- Quantized numerical traffic tokens
- Semantic text embeddings
- Temporal information
- Sensor identifiers

This structure allows the future LLM backbone to interpret traffic states as a sequential language-like representation.

### 4. Sequence construction

Temporal windows were generated to create spatio-temporal sequences compatible with transformer architectures.

Each sequence contains synchronized numerical and semantic information from multiple sensors across consecutive timestamps.

### 5. Exported artifacts

The preprocessing pipeline from Phase 2 generates the following outputs:

| File | Description |
|---|---|
| `tokenized_speed.npy` | Quantized traffic speed tokens |
| `semantic_embeddings.npy` | Vector embeddings of incident descriptions |
| `multimodal_sequences.npy` | Combined numerical + semantic temporal sequences |
| `tokenizer_config.json` | Quantization/token configuration |

These artifacts constitute the direct input for the multimodal LLM training stage.
...
```bash
pip install pandas numpy matplotlib seaborn scipy h5py tables sentence-transformers transformers torch
```
...
# Copy PEMS-BAY.csv, adj_mx_bay.pkl,
# PEMS-BAY-META.csv and US_Accidents_March23.csv into data/

python fase1_pipeline_REAL.py
python fase2_tokenization_embeddings.py
```
...
Transformation of numerical traffic variables and incident descriptions into multimodal sequential representations compatible with LLM architectures.
